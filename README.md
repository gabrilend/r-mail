...
**Messages not sending** — check that the contact is in `~/mail/contacts`, the host/port are reachable, and both sides have the same token.

**Port already in use** — another instance may be running, or change the port in `~/mail/config`.

**Firewall** — make sure port 8025 (or your configured port) is open for TCP on both sides.
