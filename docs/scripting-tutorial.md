...
## Shell notification via new-mail file

rmail includes a simple notification system that works without a desktop notification daemon. It uses a file at `~/mail/.state/new-mail` to accumulate notifications.

### How it works

1. **On startup**, rmail creates an empty `new-mail` file
2. **When mail arrives**, a hook appends a line to the file
3. **When you open a shell**, a script checks the file and prints any notifications

### Setup

Use the included scripts in `scripts/`:

**on-message.sh** — hook that logs new messages:
```sh
#!/bin/sh
...
```
on_receive = /path/to/scripts/on-message.sh
```

**on-package.sh** — hook that logs received attachments:
```sh
#!/bin/sh
NOTIFY_FILE="${HOME}/mail/.state/new-mail"
echo "attachment $2 from $1" >> "$NOTIFY_FILE"
```

Configure:
```
on_package = /path/to/scripts/on-package.sh
```

**check-mail.sh** — source this in your shell rc file:
```sh
NOTIFY_FILE="${HOME}/mail/.state/new-mail"
...
When you open a new terminal after receiving mail:
```
you have new mail:
  message abc123-from-alice
  attachment photo.jpg from bob
```

The file is cleared after displaying, so you only see each notification once.
